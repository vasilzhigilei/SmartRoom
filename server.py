...
# initial receive data and recognization so that there is a frame and img to work with (a better solution will have
# to be implemented
...
def receive_data():
    """
    Multithreading function that receives client data and extracts the pickled frame.
    """
    global data, payload_size, conn, frame
    while True:
        while len(data) < payload_size:
            data += conn.recv(4096)
        ...
        preframe=pickle.loads(frame_data, fix_imports=True, encoding="bytes")
        frame = cv2.imdecode(preframe, cv2.IMREAD_COLOR)

def recognize_faces():
    """
    Multithreading function that recognizes faces in a frame. The function then makes an image with identified faces
    boxed in. This image is then displayed in the tkinter GUI.
    """
    global recognizer, font, faceCascade, img
    while True:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
...
t2 = threading.Thread(target=recognize_faces)
t1.start()
t2.start()
